...
Exit:   #Does as expected: will exit the program
   li $v0, 10
   syscall
...
inSort:
	...
	move $t0, $a0 #elements
   	move $t1, $a1 #size of the array
    	li $t2, 1 #loop counter at 1

sort1:
    move $t0, $a0    #sets t0 to a0 value
    beq $t2, $t1, sortE2 #if t2 and t1 are the same, ends/goes to final
    move $t3, $t2     #set t3 to stored value of t2

sort2:
    move $t0, $a0
    mul $t4, $t3, 4 
    add $t0, $t0, $t4   #t0=t0+$t4
    beq $t3, 0, next 	#sets t3 to 0 and goes next
    lw $t5, 0($t0)	#Sets t5 to the address 0 (t0)  
    lw $t6, -4($t0)     #set t6 to address of -4(t0)
    bge $t5, $t6, next  #if t5 is greater than t6, goes to next
    lw $t7, 0($t0)
    sw $t6, 0($t0)
    sw $t7, -4($t0)
    addi $t3, $t3, -1
...
nokey:
	jr $ra	
NoZero:
	la $a0, nozero     # Nonzero error message
 	li $v0, 4	   # $v0 will be set to the print message
  	syscall
	j Exit
negative:
	la $a0, nonegative # Nonnegative error message
 	li $v0, 4	   # $v0 will be set to the print message
  	syscall
	j Exit
invalid_size:
 	la $a0, error_msg  # Wrong size error message
 	li $v0, 4	   # $v0 will be set to the print message
  	syscall